...
<body>
    ...
    <?php
        ...
        $bookAuthorName = NULL;
        $bookActivityId = NULL;
        $lendingMemberId = NULL;
        $memberId = NULL;
        $memberName = NULL;
        $memberClassAllocation = NULL;
        $memberType = NULL;
        $currentBookOverdueStatus = false;
        $fineForOverdue = 0.00;
...
//        }
        ...
        if (isset($_GET['bookRefId']))
        {
            ...
            while ($lendingBookDr = mysqli_fetch_array($lendingBookInfo))
            {
                ...
                //$lendingMemberId = $lendingBookDr['member_id'];
            }
            
            $sql_getLendingMemeberId = "select book_activity_id, member_id from lib_book_activity where returned_date is null and record_status = 'ACTIVE' and book_reference_id = $bookReferenceID;";
            $lendingMemberIdRs = executeSqlQuery($sql_getLendingMemeberId);
            
            while ($lendingMemberIdDr = mysqli_fetch_array($lendingMemberIdRs))
            {
                $lendingMemberId = $lendingMemberIdDr['member_id'];
                $bookActivityId = $lendingMemberIdDr['book_activity_id'];
            }
            ...
            while ($lendingMemberDr = mysqli_fetch_array($lendingMemberInfo))
            {
                ...
                $memberName = $lendingMemberDr['member_name'];
                $memberClassAllocation = $lendingMemberDr['class_allocation'];
                $memberType = $lendingMemberDr['member_type'];
            }
        }
        
        if (isset($lendingMemberId) && isset($_GET['bookRefId']))
        {
            $sql_CheckBookOverdueState = "select lib_book_activity.member_id, lib_book_activity.book_reference_id, inv_books.book_title_en, inv_books.author_name, lib_book_activity.lending_date, datediff(curdate(), lib_book_activity.lending_date) as days_in_loan from lib_book_activity inner join inv_books on lib_book_activity.book_reference_id = inv_books.reference_id where lib_book_activity.returned_date is null and lib_book_activity.record_status = 'ACTIVE' and lib_book_activity.book_reference_id = ".trim($_GET['bookRefId'])." and lib_book_activity.member_id = $lendingMemberId;";
            $isBookOverdueRs = executeSqlQuery($sql_CheckBookOverdueState);
            if(mysqli_num_rows($isBookOverdueRs) > 0)
            {
                while($isBookOverdueDr = mysqli_fetch_array($isBookOverdueRs))
                {
                    $bookOverdueDays = $isBookOverdueDr['days_in_loan'];
                }
                
                if (($memberType == 'STUDENT') && ($bookOverdueDays >= 15))
                {
                    $currentBookOverdueStatus = true;
                    $fineForOverdue = $bookOverdueDays - 14;
                }
                else
                {
                    $currentBookOverdueStatus = false;
            ...
            $sql_getBooksWithMember = "select lib_book_activity.member_id, lib_book_activity.book_reference_id, inv_books.book_title_en, inv_books.author_name, lib_book_activity.lending_date, datediff(curdate(), lib_book_activity.lending_date) as days_in_loan from lib_book_activity inner join inv_books on lib_book_activity.book_reference_id = inv_books.reference_id where lib_book_activity.returned_date is null and lib_book_activity.record_status = 'ACTIVE' and lib_book_activity.member_id = $lendingMemberId;";      
    ...
    <!-- Sidebar ends -->
  	...
  	<div class="mainbar">
        ...
        <!-- Page heading -->
          ...
          <!-- page meta -->
          <span class="page-meta">Return books</span>
        </h2>
            ...
            <div class="span12">
              <div class="widget wgreen">
                <div class="widget-head">
                  <div class="pull-left">Book Returns <?php if($currentBookOverdueStatus == true){echo '<span class="label label-important">Overdue</span>';} ?></div>
                ...
                <div class="widget-content">
                  <div class="padd">
                      ...
                      <form class="form-horizontal" id="frmRentBookMemberInput" method="post" action="libReturnBookProcessor.php">
                          <div class="row-fluid">
                              <div class="span6">
                                  ...
                                  <?php
                                    ...
                                    if ($currentBookOverdueStatus == true)
                                    {
                                  ?>
                                  <div class="control-group">
                                    <label class="control-label" for="inputFineForOverdue">Fine for Overdue</label>
                                    <div class="controls">
                                        <input type="text" id="inputFineForOverdue" name="inputFineForOverdue" placeholder="Fine" value="<?php echo number_format($fineForOverdue, 2); ?>" readonly>
                                        <label><input type="checkbox" id ="inputSaveFine" name="inputSaveFine">Add Fine</label>
                                    </div>
                                  </div>
                                  <?php
                                    }
                                  ...
                                  <a class="btn btn-link" href="libHome.php"><i class="icon-arrow-left"></i> Is this Information Incorrect?</a>
                              </div>
                              <div class="span6">
                                  <div class="control-group">
                                    <label class="control-label" for="inputBookReferenceID">Reference #</label>
                                    <div class="controls">
                                        <input type="hidden" id="inputBookActivityId" name="inputBookActivityId" value="<?php echo $bookActivityId; ?>">
                            ...
                            <button type="submit" id="btnReturnBook" name="btnReturnBook" class="btn btn-success">Return</button>
                ...
                <div class="widget-foot">
                    <!-- Footer goes here -->
                    <?php
                        if (isset($_POST['btnAddRent']))
                        {
                            echo $publicationName;
                            echo $publicationType;
                            echo $publicationFrequency;
                            echo $publicationPrice;
                        }
                    ?>
                </div>
