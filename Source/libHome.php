...
<body>
    ...
    <?php
        ...
        {
            $loanBookReferenceId = trim($_POST['inputBookReferenceID']);
            
            if (($loanBookReferenceId == "") or ($loanBookReferenceId == NULL))
            {
                $errorMessage = "Cannot Empty";
            }
            else
            {

                $bookStatus = getLendingBookStatus($loanBookReferenceId);

                if ($bookStatus == 2)
                {
                    header('Location: libRent.php?bookRefId='.$loanBookReferenceId);
                    exit();
                }
                else if ($bookStatus == 1)
                {
                    header('Location: libRentReturns.php?bookRefId='.$loanBookReferenceId);
                    exit();
                }
                else
                {
                    $errorMessage = "You have entered an incorrect book reference ID";
                }
            }
                ...
                <div class="widget-content">
                  <div class="padd">
                      ...
                      <?php
                            ...
                            {
                                $msgAry = explode('|', $_SESSION['ses_ErrorMessage'],2);
                                $msgType = $msgAry[0];
                                $msgString = $msgAry[1];
                                
                                if ($msgType == 1)
                                {
                                    echo '<div class="alert alert-success">'.$msgString.'</div>';
                                    unset($_SESSION['ses_ErrorMessage']);
                                }
                                else
                                {
                                    echo '<div class="alert alert-error">'.$msgString.'</div>';
                                    unset($_SESSION['ses_ErrorMessage']);
                                }
                            }
                      ?>
                      
                      <form class="form-horizontal" id="frmBookReference" method="post" action="libHome.php">
                          <div class="row-fluid">
                              <div class="span12">
                                  <div class="control-group">
                                    <label class="control-label" for="inputBookReferenceID">Reference #</label>
                                    <div class="controls">
                                        <input type="text" id="inputBookReferenceID" name="inputBookReferenceID" placeholder="Reference ID" required>
                                    </div>
                                  </div>
                            <button type="submit" id="btnProceedBook" name="btnProceedBook" class="btn btn-success">Proceed</button>
